...
type Config = {
  ...
  pipelineBatchWrites?: boolean // when true with pipeline, coalesces writes from the same turn. Default false.
}
```
...
## client.pipelineBatchWrites

`client.pipelineBatchWrites: boolean` (read-only)

Whether this client coalesces writes issued during the same synchronous turn when pipelining is enabled. Set via
`pipelineBatchWrites: true` together with `pipeline: true`. Defaults to `false` and has no effect without pipelining.
Each pipelined query still sends its own `Sync` message.
